...
export default function Home() {
  ...
  const addHabit = () => {
    ...
    const newHabit: Habit = { id: Date.now(), name: customHabit, icon: "⭐" };
    ...
    <main className="min-h-screen bg-gradient-to-br from-purple-50 to-indigo-100 p-6">
      <div className="max-w-md mx-auto">
        ...
        <div className="space-y-3 mb-6">
          {habits.map((habit) => (
            <div key={habit.id} className={`flex items-center justify-between p-4 rounded-2xl shadow transition-all ${completed[habit.id] ? "bg-indigo-500 text-white" : "bg-white text-gray-700"}`}>
              <button onClick={() => toggle(habit.id)} className="flex items-center gap-3 flex-1 text-left">
                <span className="text-2xl">{habit.icon}</span>
                <span className={`font-medium ${completed[habit.id] ? "line-through opacity-80" : ""}`}>{habit.name}</span>
              </button>
        ...
        <div className="bg-white rounded-2xl p-4 shadow">
          ...
          <div className="flex gap-2">
            <input type="text" value={customHabit} onChange={(e) => setCustomHabit(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addHabit()} placeholder="e.g. Journal writing..." className="flex-1 border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300" />
            <button onClick={addHabit} className="bg-indigo-500 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-indigo-600">Add</button>
          </div>
        </div>
      </div>
    </main>
